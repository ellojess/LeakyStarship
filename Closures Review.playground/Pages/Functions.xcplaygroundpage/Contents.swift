...
/*:
 ...
 Functions and Closures are a fundamental part of implementing iOS apps. They're the building blocks that we use for everything we do.
 ...
 1. Review what a function is in Swift and how to write and call them.
 1. Describe what an instance method or class method is, and know when to use them.
 1. Review what a closure is and how to write and call them.
 1. Know when it makes sense to use a closure in your projects.
 1. Review reference and value types, and what it means to pass by reference vs value
 
 # Functions
 
 **Functions** are the building blocks of coding in any language.
...
/*:
 ...
 Say you are hosting an extravagant dinner party (or video game night, or slam poetry reading, it's your event!). Every host will need a list of guests attending the event to know how much food to buy, what entertainment to have, and if Lenny is going to be there so you know to hide your water bottle that he always steals...
 ...
 - note: Guests conform to two protocols `CustomDebugStringConvertible` and `Equatable`.
...
func getAgeAndName(guest: Guest){
    print("\(guest.name) is \(guest.age)")
}

//Call the function and pass in Eric into the function
getAgeAndName(guest: eric)

/*:
 Let's invite more guests! As much as we love Eric, we can only handle close-up magic for so long.
 
 Let's invite Paola, Sven, and Shuyi. We're going to want to and store these guests into an array.
 */

let paola = Guest(name: "Paola", age: 24)
let sven = Guest(name: "Sven", age: 23)
let shuyi = Guest(name: "Shuyi", age: 17)
let invitedGuests = [paola, eric, sven, shuyi]

/*:
 Now that we have our array of invited guests, let's see if we can get some more info about them.
 1. Write a function that takes in an array of guests and prints each guest's name (hint: the parameter's type will look like this `[Guest]`)
...
/*:
 Great! Now that we have our list of invited guests we can see each of their ages. Except now we realize that Shuyi is still 17! Het birthday isn't for another couple of months. Since we'll be having guests ranging from teens to adults, we need to see who's 18 years or older so we know who doesn't need a parent to sign a liability waiver (you know... these people like to party hard).
...
let dan = Customer(firstName: "Dan", lastName: "Rodriguez", age: 20)
let timmy = Customer(firstName: "Timmy", lastName: "Turner", age: 18)
